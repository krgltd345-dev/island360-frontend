  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-sm text-slate-600">{bookings.length} total bookings</span>
      </div>
      ...
      {bookings.map((booking) => (
        <Card key={booking.id} className="p-6 hover:shadow-md transition-shadow">
            ...
            <div className="flex-1">
              ...
              {booking.special_requests && (
                <div className="mt-3 p-3 bg-slate-50 rounded-lg">
                  ...
                  <p className="text-sm text-slate-800">{booking.special_requests}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100 text-xs text-slate-500">
            ...
            <span>Booked on {format(parseISO(booking.created_date), 'MMM d, yyyy')}</span>
          </div>
        </Card>
      ))}
    </div>
